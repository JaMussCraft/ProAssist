...
from mmassist.datasets.prepare.prompts import (
    image_short_description_prompts as system_prompts,
)
...
class PreprocessArgs:
    ...
    data_dir: str = f"{DATA_ROOT_DIR}/processed_data/llava"
    output_dir: str = f"{DATA_ROOT_DIR}/processed_data/llava/prepared"
    prefix: str = "caption_train"
...
if __name__ == "__main__":
    ...
    def process_video(ann_file: str) -> list[PreparedSample]:
        ...
        for idx, cap in enumerate(ann["captions"]):
            # process a caption
            if cap[-1] not in [".", "?", "!"]:
                cap += "."
            turns = [
                {"role": "frames", "start": idx, "end": idx + 1},
                {"role": "assistant", "content": cap.capitalize()},
            ]
